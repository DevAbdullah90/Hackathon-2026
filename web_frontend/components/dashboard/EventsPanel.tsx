...
> = {
  ...
  detection_agent: {
    label: "Crisis Clusterer",
    color: "text-purple-700",
    bg: "bg-purple-50",
    border: "border-purple-200",
  },
  verification_agent: {
    label: "Social Verifier",
    color: "text-amber-700",
    bg: "bg-amber-50",
    border: "border-amber-200",
  },
  severity_agent: {
    label: "Severity Indexer",
    color: "text-red-700",
    bg: "bg-red-50",
...
export default function EventsPanel({
  ...
  const [activeTab, setActiveTab] = useState<
    "actions" | "logs" | "notifications"
  >("actions");
  const [subTab, setSubTab] = useState<
    "public" | "police" | "utility" | "hospital"
  >("public");
  ...
  useEffect(() => {
    if (selectedIncident) {
      ...
      const allCompleted =
        hasActions && actions.every((a) => a.status === "COMPLETED");
      const anyActive =
        hasActions &&
        actions.some((a) =>
          [
      ...
      if (allCompleted) {
        ...
        if (
          statuses.includes("ON_SITE") ||
          statuses.includes("ON-SITE") ||
          statuses.includes("ON SITE")
        ) {
          cong = 35;
          safety = 85;
  ...
  useEffect(() => {
    const handleSimulationProgress = (e: Event) => {
      ...
      if (
        data &&
        selectedIncident &&
        data.incident_id === selectedIncident.id
      ) {
        ...
        if (data.metrics) {
          ...
          const safety =
            data.metrics.cooling_coverage ||
            data.metrics.safety_rate ||
            data.metrics.evacuation_rate;
  ...
  const handleRunSimulation = async () => {
    ...
    if (!success) {
      setIsSimulating(false);
      alert(
        "Failed to start simulation. Verify backend is running and actions exist for this incident.",
      );
    }
  };
    ...
    return (
      <div className="flex flex-col space-y-2 py-1">
        <div className="flex justify-between items-center text-[10px] font-bold">
          ...
          <span
            className={`font-mono px-2 py-0.5 rounded-sm text-[9px] font-bold uppercase ${
              congestionHistory[congestionHistory.length - 1] > 50
                ? "bg-red-50 text-red-600 border border-red-200"
                : "bg-emerald-50 text-emerald-600 border border-emerald-200"
            }`}
          >
            Current: {congestionHistory[congestionHistory.length - 1]}% (
            {congestionHistory[congestionHistory.length - 1] > 50
              ? "Gridlock"
              : "Flowing"}
            )
        ...
        <div className="bg-slate-950 rounded-xl p-3 border border-slate-900 shadow-inner flex items-center justify-center">
          ...
          >
            ...
            <text
              ...
              y={paddingY + 3}
              textAnchor="end"
              className="text-[8px] font-mono fill-gray-500 font-bold"
            >
              90%
            </text>
            <text
            ...
            {points.map((p, idx) => (
              <g key={idx}>
                <circle
                  ...
                  className={
                    idx === points.length - 1
                      ? "fill-emerald-400 stroke-slate-950 stroke-1.5"
                      : "fill-red-500 stroke-slate-950 stroke-1.5"
                  }
                />
                {idx === points.length - 1 && (
                  <circle
                    ...
                    cy={p.y}
                    r="8"
                    className="fill-none stroke-emerald-400 stroke-1 animate-ping"
                  />
                )}
              </g>
            ))}
            ...
            <text
              x={paddingX + (width - 2 * paddingX) * 0.33}
              y={height - 4}
              textAnchor="middle"
              className="text-[8px] font-mono fill-gray-500 font-bold"
            >
            ...
            <text
              x={paddingX + (width - 2 * paddingX) * 0.66}
              y={height - 4}
              textAnchor="middle"
              className="text-[8px] font-mono fill-gray-500 font-bold"
            >
      ...
      <div className="flex items-center gap-6 py-2">
        <div className="relative w-24 h-24 flex items-center justify-center flex-shrink-0">
          ...
          <div className="absolute flex flex-col items-center justify-center">
            ...
            <span className="text-[8px] font-extrabold text-gray-400 uppercase tracking-wider mt-0.5">
              Coverage
            </span>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col h-full">
      ...
      <div
        className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${isHeatwave ? "from-amber-500 to-orange-600" : "from-teal-600 to-emerald-600"}`}
      />
      {/* Card Header */}
        ...
        {selectedIncident ? (
          <div className="space-y-4">
            ...
            >
              <div className="flex items-center gap-2.5">
                <div
                  className={`w-9 h-9 rounded-lg flex items-center justify-center border shadow-sm flex-shrink-0 ${
                    isHeatwave
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((lvl) => {
                    const active =
                      lvl <= getEmergencyLevel(selectedIncident.severity_score);
                    return (
                      <div
                        key={lvl}
              ...
              <div className="space-y-3">
                <div className="text-xs text-gray-600 leading-relaxed font-semibold bg-gray-50 border border-gray-200/60 p-4 rounded-xl shadow-inner min-h-[140px] flex flex-col justify-between">
                  <div>
                    ...
                    {isHeatwave ? (
                      <>
                        ResQ by AQUA verified an active extreme heatwave warning
                        in {selectedIncident.location} at coordinate [
                        {selectedIncident.lat.toFixed(4)},{" "}
                        {selectedIncident.lng.toFixed(4)}]. The severity index
                        is evaluated to {selectedIncident.severity_score}/10,
                        placing approximately{" "}
                        {selectedIncident.estimated_population.toLocaleString()}{" "}
                        citizens at critical risk of thermal shock and grid
                        outages. Hydration deployment and cooling systems have
                        been scheduled.
                      </>
                    ) : (
                      <>
                        ResQ by AQUA verified an active flood warning in{" "}
                        {selectedIncident.location} at coordinate [
                        {selectedIncident.lat.toFixed(4)},{" "}
                        {selectedIncident.lng.toFixed(4)}]. The severity index
                        is evaluated to {selectedIncident.severity_score}/10,
                        exposing approximately{" "}
                        {selectedIncident.estimated_population.toLocaleString()}{" "}
                        citizens at high risk. Tactical resource allocation and
                        emergency evacuation procedures have been successfully
                        finalized.
                      </>
                    )}
                  </div>
              ...
              <div className="border border-gray-100 p-3.5 rounded-xl bg-white shadow-sm space-y-1.5 text-xs">
                ...
                <div className="flex justify-between py-1 border-b border-gray-50">
                  ...
                  <span className="font-semibold text-gray-800">
                    {selectedIncident.affected_radius_km} km
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="text-gray-500 font-medium">
                  ...
                  >
                    {selectedIncident.risk_factors?.length
                      ? selectedIncident.risk_factors.join(", ")
                      : isHeatwave
                        ? "Extreme Heat"
                        : "Heavy Rain"}
                  </span>
                ...
                <div className="pt-3 mt-2 border-t border-gray-100 space-y-2">
                  ...
                  <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden flex">
                    {(selectedIncident.confirmations_count || 0) +
                      (selectedIncident.refutations_count || 0) >
                    0 ? (
                      <>
                        <div
                  ...
                  <div className="flex justify-between text-[9px] text-gray-400 font-medium">
                    ...
                    {(selectedIncident.refutations_count || 0) > 0 && (
                      <span className="text-rose-500 font-bold">
                        {selectedIncident.refutations_count} /{" "}
                        {(selectedIncident.confirmations_count || 0) + 3}{" "}
                        towards retraction
                      </span>
                    )}
                  </div>
                ...
                loadingActions ? (
                  ...
                  <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                    {actions.map((act) => (
                      <div
                        key={act.id}
                        className={`flex flex-col md:flex-row md:items-center justify-between p-2.5 bg-white border border-gray-200 rounded-lg gap-2 transition-colors shadow-2xs ${
                          isHeatwave
                      ...
                      >
                        ...
                        <span
                          className={`text-[9px] font-extrabold uppercase px-2 py-0.5 border rounded-md self-start md:self-center tracking-wider ${getActionBadgeColor(act.status)}`}
                        >
                          {act.status}
                        </span>
                      </div>
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                    {reasoningLogs.map((log) => {
                      ...
                      return (
                        <div
                          key={log.id}
                          className={`flex flex-col p-3 bg-white border border-gray-200 rounded-xl gap-2 shadow-2xs transition-colors ${
                            isHeatwave
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span
                              className={`text-[10px] font-extrabold uppercase px-2 py-0.5 border rounded-md tracking-wider ${meta.bg} ${meta.color} ${meta.border}`}
                            >
                              {meta.label}
                            </span>
                      ...
                      if (filtered.length === 0) {
                        return (
                          <div className="text-xs text-gray-400 py-12 text-center font-medium italic w-full">
                            Awaiting message generation from Notification
                            Agent...
                          </div>
                        );
                      }
                          ...
                          {filtered.map((item) => (
                            ...
                            >
                              <div className="flex items-center justify-between mb-1.5">
                                <span
                                  className={`text-[9px] font-mono font-extrabold uppercase px-1.5 py-0.5 rounded-sm border ${
                                    isHeatwave
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <span className="text-sm font-bold uppercase tracking-wider">
              No Active Crisis Hazards
            </span>
            <p className="text-xs text-gray-400 mt-1 max-w-[240px] text-center font-medium">
              Initiate simulated emergency telemetries in the top bar to watch
              the orchestrator run live!
            </p>
          </div>
        )}
      </div>
